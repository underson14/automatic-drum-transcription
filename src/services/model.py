...
#     X = X[:,:,:,:3]
    ...
    X_train, X_test = X_train.reshape((X_train.shape[0], 224, 224, 1)), X_test.reshape((X_test.shape[0], 224, 224, 1))
#     y_train, y_test = y[:train_pct_index].reshape((X.shape[0], 224, 224, 1)), y[train_pct_index:].reshape((X.shape[0], 224, 224, 1))
#     y_train, y_test = y[:train_pct_index].reshape((X.shape[0], 224, 224, 1)), y[train_pct_index:].reshape((X.shape[0], 224, 224, 1))
#     X_train, X_test = X[:train_pct_index].reshape((X.shape[0], 224, 224, 1)), X[train_pct_index:].reshape((X.shape[0], 224, 224, 1))
    ...
    model = Sequential()
    model.add(Conv2D(input_shape=(224,224,1),filters=64,kernel_size=(88,3),padding="same", activation="relu"))
